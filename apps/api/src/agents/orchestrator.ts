...
export async function runBuildProofAgents(projectId: string, submission: ProjectSubmission, repo: GitHubInspection): Promise<BuildProofReport> {
  const findings: Record<string, AgentFinding> = {
    ...
    CommunityMentor: communityFinding(submission)
  };
  ...
  const draft: BuildProofReport = {
    ...
    verifiedModules: hasValid0GProof ? ["0G Chain"] : [],
    scores: { ...rawScores, overall: capped.score },
    ...
    judgeSummary: `${submission.projectName} is ${capped.score >= 80 ? "judge-ready" : "promising but needs follow-up"} based on the current BuildProof evidence bundle.`,
  ...
  const storage = await uploadReportTo0G(draft, submission.targetNetwork);
  draft.evidence.storageRoot = storage.root;
  draft.evidence.storageTxHash = storage.txHash;
  if (storage.mode === "0g-storage") draft.verifiedModules.push("0G Storage");

  draft.evidence.reportHash = reportHash(draft);
  const chain = await anchorReportOn0G({
    network: submission.targetNetwork,
  ...
  if (chain.mode === "0g-chain") draft.verifiedModules.push("0G Chain");
  if (draft.agentFindings.CommunityMentor) draft.verifiedModules.push("0G Compute");
...
function communityFinding(submission: ProjectSubmission): AgentFinding {
  return {
    agentName: "CommunityMentor",
    score: 86,
    summary: "Project can contribute useful proof and quality signals to the 0G ecosystem.",
    evidence: [submission.projectName, ...submission.claimedModules],
    warnings: [],
    criticalIssues: [],
    recommendedFixes: [
